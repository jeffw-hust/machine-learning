...
max_words = 3000
epochs = 50
poetry_file = '/Users/jeffw/.dataset/rnn/poetry.txt'
save_dir = '/Users/jeffw/workspace/jeffwang/github/machine-learning/sample/rnn/poetry_generator/model/'
    ...
    def load(self):
        def handle(aline):
            if len(aline) > MAX_LENGTH:
                index_end = aline.rfind('。', 0, MAX_LENGTH)
                index_end = index_end if index_end > 0 else MAX_LENGTH
                aline = aline[:index_end + 1]
            return BEGIN_CHAR + aline + END_CHAR

        self.poetries = [line.strip().replace(' ', '').split(':')[1] for line in
                         open(self.poetry_file)]
        self.poetries = [handle(line) for line in self.poetries if len(line) > MIN_LENGTH]
        # 所有字
        words = []
        ...
        self.poetries = sorted(self.poetries, key=lambda line: len(line))
        self.poetrys_vector = [list(map(self.char2id, poetry)) for poetry in self.poetries]
